...
export default function DriverHome() {
  ...
  async function fetchDriverDetails() {
    ...
    if (data) {
      // If driver is marked offline but has no active ride, restore them to online
      // This handles the case where a student cancels and driver returns to home
      let available = data.is_available
      if (!available) {
        const { data: activeRide } = await supabase.from('rides')
          .select('id').eq('driver_id', profile.id)
      ...
      setDriverDetails({ ...data, is_available: available })
      setIsAvailable(available)
      if (available) startLocationTracking()
    }
  }
